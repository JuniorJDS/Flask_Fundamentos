...
def cadastrar_cliente():
    ...
    if form.validate_on_submit():
        ...
        cli = cliente_model.Cliente(nome=nome, email=email, data_nascimento=data_nascimento,
                                    profissao=profissao, sexo=sexo)
        try:
            cliente_service.cadastrar_cliente(cli)
...
def listar_cliente(id):
    cliente_list = cliente_service.listar_cliente(id)
    return render_template("clientes/lista_cliente.html", cliente=cliente_list)


@app.route("/editar_cliente/<int:id>", methods=["POST", "GET"])
def editar_cliente(id):
    cliente_bd = cliente_service.listar_cliente(id)
    form = cliente_form.ClientForm(obj=cliente_bd)
...
def remover_cliente(id):
    ...
    if request.method == "POST":
        try:
            cliente_service.remover_cliente(cliente_list)
            return redirect(url_for("listar_clientes"))
        except:
            print("Cliente não foi Removido!")

    return render_template("clientes/remover_cliente.html", cliente=cliente_list)
